...
def _build_strategy(strategy_name: str):
    if strategy_name == "KDJ金叉死叉":
        period = st.number_input("KDJ周期", min_value=2, max_value=120, value=9, step=1)
        k_smooth = st.number_input("K平滑参数", min_value=1, max_value=30, value=3, step=1)
        d_smooth = st.number_input("D平滑参数", min_value=1, max_value=30, value=3, step=1)
        return KDJStrategy(period, k_smooth, d_smooth)
    if strategy_name == "MACD金叉死叉":
        fast = st.number_input("fast", min_value=2, max_value=120, value=12, step=1)
        slow = st.number_input("slow", min_value=3, max_value=240, value=26, step=1)
        signal = st.number_input("signal", min_value=2, max_value=120, value=9, step=1)
...
def main() -> None:
    ...
    try:
        with st.spinner("正在获取行情并回测..."):
            ...
            result = BacktestEngine(initial_cash, config).run(data, strategy, symbol=symbol)
    ...
    st.write(f"回测区间：{start_date.isoformat()} 至 {end_date.isoformat()}")
